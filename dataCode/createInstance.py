...
def create_instance(fileName: str, limit: int=900, couriersPerWarehouse: int=5):
    ...
    warehouses = np.c_[warehouses, np.ones(len(warehouses))*couriersPerWarehouse]
    print(warehouses)
    ...
    with open("instances/"+fileName+".txt", 'w') as f:
        ...
        f.write("\n".join([
            "{}\t{}\t{}\t{}".format(i, y, z, int(q))
            for i, (y, z, q) in enumerate(warehouses)
        ]))
        f.write("\n")
...
if __name__ == "__main__":
    create_instance(fileName = "instance900", limit=900, couriersPerWarehouse=5)